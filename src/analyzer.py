...
def load_csv(file_path: str) -> pd.DataFrame:
    """
    eine CSV-Datei mit automatischer Erkennung von Encoding und Trennzeichen.
    """
    file_path = Path(file_path)
    if not file_path.exists():
        raise FileNotFoundError(f"Datei nicht gefunden: {file_path}")

    """Versuchen Sie bitte verschiedene Encodings (häufigste zuerst)"""
    ...
    """Fallback: Pandas automatisch erkennen lassen"""
    try:
